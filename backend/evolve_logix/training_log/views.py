...
from .models import Exercise, TrainingLog, ExerciseInSession, Set, TrainingSession
from rest_framework import status
from .serializers import ExerciseSerializer, TrainingLogSerializer
...
class SetsPerMuscleGroupView(APIView):
    def get(self, request):
        ...
        training_sessions = TrainingSession.objects.filter(training_log__user=request.user, date__gte=one_week_ago)

        # Get the ExercisesInSession for these TrainingSessions
        exercises_in_session = ExerciseInSession.objects.filter(training_session__in=training_sessions)
        ...
        sets_per_muscle_group = sets.values('exercise_in_session__exercise__muscle_group__name').annotate(count=Count('id'))
